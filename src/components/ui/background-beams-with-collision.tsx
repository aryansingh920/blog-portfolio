...
export const BackgroundBeamsWithCollision = ({
  ...
  return (
    <div
      ref={parentRef}
      className={cn(
        "h-96 bg-linear-to-b from-white to-neutral-100 dark:from-neutral-950 dark:to-neutral-800 relative flex items-center w-full justify-center overflow-hidden",
        className
      )}
    >
      {beams.map((beam, idx) => (
        <CollisionMechanism
          key={`${String(beam.initialX)}-${idx}`}
          beamOptions={beam}
...
function CollisionMechanism({
  ...
  useEffect(() => {
    const checkCollision = () => {
      ...
      if (!beamEl || !containerEl || !parentEl || cycleCollisionDetected)
        return;
  ...
  const initialY = beamOptions.initialY ?? "-200px";
  const translateY = beamOptions.translateY ?? "1800px";
  const initialX = beamOptions.initialX ?? "0px";
  const translateX = beamOptions.translateX ?? "0px";
  ...
  return (
    <>
      <motion.div
        ...
        ref={beamRef}
        animate="animate"
        initial={{ translateY: initialY, translateX: initialX, rotate }}
        variants={{ animate: { translateY, translateX, rotate } }}
        transition={{
